...
// TODO 4.4a-d: Extend this class by defining (non-abstract) subclasses `Blinky`, `Pinky`, `Inky`,
 ...
 * <p>
 * - WAIT: The ghost is not currently present in the maze, its `initialDelay` specifies how long before it
 * enters.
 * <p>
...
public abstract class Ghost extends Actor {
    ...
    /**
     * When in the WAIT state, the number of milliseconds remaining before this ghost returns to its
     * CHASE state
     */
    ...
    /**
     * The edges comprising the most recently calculated path to this ghost's `target()`
     */
    ...
    public DPair getBoundingBoxUL() {
        if (state == GhostState.WAIT) {
            IPair gStartLoc = location.edge().tail().loc();

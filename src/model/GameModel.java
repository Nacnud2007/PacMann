...
public class GameModel {
    ...
    /**
     * Each path tile contains a DOT, contains a PELLET, or is empty (NONE).
     */
    public enum Item {DOT, PELLET, NONE}

    /**
     * A map from the vertices in the game graph to items positioned on these vertices. `items` only
    ...
    /**
     * Stores the direction of the latest player input.
     */
    private Direction playerCommand;

    /**
     * Returns the direction of the most recent player input.
     */
    ...
    /**
     * Updates playerCommand to the direction of the most recent input.
     */
    public void updatePlayerCommand(Direction command) {
    ...
    /**
     * Handle a victorious end of the game (i.e., when PacMann has eaten all the DOTs and
    ...
    /**
     * Removes all property change listeners from this game model.
     */
...
//                System.err.println("Stepping " + dt + " from " + t + " (goal: " + ms + ")");
                ...
                for (Actor a : actors) {
                    if (a.location().progress() == 1) {
                        a.visitVertex(a.location().edge().head());
                    }
                }
    ...
    private double nextCollisionTime() {
        ...
        for (Actor a : actors) {
            ...
            UnorderedPair<MazeVertex> ue = new UnorderedPair<>(e.tail(), e.head());
